...
module.exports = inherit(QEmiiter, {
    ...
    _runSession: function(suites) {
        log('run session. suites amount: %d', suites.length);
    ...
    _runSuitesInBrowser: function(suites, browserId) {
        log('run suites: %d in browser: %s', suites.length, browserId);
    ...
    _tryToRunSuiteInBrowser: function(suite, browserId, retryCount) {
        log('try to run suite: %s in browser: %s', suite.fullName, browserId);
    ...
    _runSuiteInBrowser: function(suite, browser) {
        log('run suite: %s in browser: %s', suite.fullName, browser.id);
    ...
    _runSuiteStates: function(suite, browser) {
        log('run suite: %s states in browser: %s', suite.fullName, browser.id);
    ...
    _runStateInSession: function(state, session) {
        log('run single state: %s in session: %s', state.fullName, session.browserId);
